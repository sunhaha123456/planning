...
//            }
    ...
    public void startApply(Long templateId, String instanceName, String applyContent, List<MultipartFile> fileList) throws IOException {
        ...
        Iterable<TbFlowInstanceNode> flowInstanceNodeDbRetIterable = flowInstanceNodeRepository.saveAll(flowInstanceNodeListDbParam);
        ...
        flowInstanceNodeUserTaskRepository.saveAll(flowInstanceNodeUserTaskListDbParam);
        ...
        if (fileList != null && fileList.size() > 0) {
            ...
            if (flowInstanceFileList.size() > 0) {
                flowInstanceFileRepository.saveAll(flowInstanceFileList);
            }
        }
    ...
    public void approvalApply(ApprovalApplyRequest param) {
        ...
        List<Long> handingTaskNodeWaitingOperateTaskIdList = new ArrayList<>();
        ...
        for (TbFlowInstanceNodeUserTask t : handingTaskNodeTaskList) {
            if (t.getId().equals(userTaskId)) {
                ...
                if (t.getState() == FlowInstanceNodeUserTaskStateEnum.WAITINT_OPERATE.getIndex()) {
                    handingTaskNodeWaitingOperateTaskIdList.add(t.getId());
                }
            }
        }
        ...
        StringBuilder handingInstanceNodeIdsExcludeCurrentNodeId = new StringBuilder();
        ...
        String strTemp = handingTaskNode.getId() + "";
        for (String nid : handingInstanceNodeIdArr) {
            handingInstanceNodeIdList.add(Long.valueOf(nid));
            if (!strTemp.equals(nid)) {
                handingInstanceNodeIdsExcludeCurrentNodeId.append(nid).append(",");
            }
        }
        if (handingInstanceNodeIdsExcludeCurrentNodeId.length() > 0) {
